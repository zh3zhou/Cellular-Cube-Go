...
主世界始终使用硬边界、二值 Conway Life。白色 glider 会先在画布外生成，再从边界逐步进入；第一次露出时有一帧可见预警，随后恢复致死，完全进入后并入主世界继续演化。最多同时存在三座互不重叠且相隔一格的温室；外面的 Conway 细胞不能侵入。温室若演化灭绝会直接清除；否则由 Pattern 的包围盒大小与复杂度共同决定孵化代数，并在颜色渐变完成时成为普通白色细胞、恢复致死碰撞。

## 五种奖励生态

| 颜色 | 规则 | 规则式 / 邻域 | 孵化代数 | 游戏中的性格 |
| --- | --- | --- | ---: | --- |
| 绿色 | Conway Life | `B3/S23`，Moore 8 邻域 | 12–48 | 主世界规则；奖励概率永不低于 55% |
| 紫色 | HighLife | `B36/S23`，Moore 8 邻域 | 48–144 | 额外的六邻居出生支持复制结构 |
| 橙色 | Seeds | `B2/S`，Moore 8 邻域 | 32–96 | 活细胞立即死亡，新生快速爆发 |
| 青蓝色 | Day & Night | `B3678/S34678`，Moore 8 邻域 | 48–160 | 活/死反转对称，容易形成浓密边界 |
| 黄色 | Wolfram Code 52 | `B24/S134`，von Neumann 4 邻域 | 48–160 | 十字晶格中的周期块、移动边界与复杂局部结构 |

每条规则会用当前可玩目录中 Pattern 的综合复杂度和包围盒范围做归一化：该规则最轻与最重的候选分别使用表中的最短与最长孵化代数，中间候选连续插值。
